...
const WORKFLOW = { workflowId: "workflow-8-24", correlationId: "corr-8-24", brandId: null, outputs: {}, data: { avg: 1 } };

const plannerTask = { id: "planner-8-24", name: "Plan content", description: "Plan the content pipeline", agent: "planner", inputSchema: { type: "object" }, outputSchema: { type: "object" }, dependencies: [] };
const researchTask = { id: "research-8-24", name: "Research", description: "Research modern media pipelines", agent: "research", inputSchema: { type: "object" }, outputSchema: { type: "object" }, dependencies: [] };
const writerTask = { id: "writer-8-24", name: "Write article", description: "Write the media pipeline article", agent: "writer", inputSchema: { type: "object" }, outputSchema: { type: "object" }, dependencies: [] };
const seoTask = { id: "seo-8-24", name: "Optimize article", description: "Optimize the article for search", agent: "seo", inputSchema: { type: "object" }, outputSchema: { type: "object" }, dependencies: [] };
const brandTask = { id: "brand-8-24", name: "Brand gate", description: "Gate the article for brand", agent: "brand", inputSchema: { type: "object" }, outputSchema: { type: "object" }, dependencies: [] };
const reviewTask = { id: "review-8-24", name: "Review artifact", description: "Review the produced artifact", agent: "reviewer", inputSchema: { type: "object" }, outputSchema: { type: "object" }, dependencies: [] };
const qaTask = { id: "qa-8-24", name: "QA artifact", description: "Validate the content chain", agent: "qa", inputSchema: { type: "object" }, outputSchema: { type: "object" }, dependencies: [] };
const thumbnailTask = { id: "thumbnail-8-24", name: "Generate thumbnail", description: "Generate a thumbnail for the approved article", agent: "thumbnail", inputSchema: { type: "object" }, outputSchema: { type: "object" }, dependencies: [] };
const videoTask = { id: "video-8-24", name: "Generate video", description: "Generate a video for the approved article", agent: "video", inputSchema: { type: "object" }, outputSchema: { type: "object" }, dependencies: [] };
const publishTask = { id: "publish-8-24", name: "Publish content", description: "Publish the approved content", agent: "publisher", inputSchema: { type: "object" }, outputSchema: { type: "object" }, dependencies: [] };
const analyticsTask = { id: "analytics-8-24", name: "Fetch analytics", description: "Fetch performance analytics for the published content", agent: "analytics", inputSchema: { type: "object" }, outputSchema: { type: "object" }, dependencies: [] };
const growthTask = { id: "growth-8-24", name: "Recommend growth", description: "Recommend growth actions from the analytics report", agent: "growth", inputSchema: { type: "object" }, outputSchema: { type: "object" }, dependencies: [] };
const financeTask = { id: "finance-8-24", name: "Analyze finance", description: "Analyze financial performance", agent: "finance", inputSchema: { type: "object" }, outputSchema: { type: "object" }, dependencies: [] };
...
function qaJson(status) { return { reportId: "00000000-0000-4000-8000-0000000000Q1", requestId: "qa-8-24", objective: "Validate the content chain", status, summary: "review", testResults: [], findings: [], risks: [], recommendations: [], metadata: { createdAt: "2026-08-14T00:00:00.000Z", agentVersion: "1.0.0", executionEvidencePresent: false } }; }
...
function capabilityBoundary({ publishProvider, analyticsProvider } = {}) {
  ...
  const analyticsProviderImpl = analyticsProvider ?? {
    fetch: async () => ({ providerId: "fake-analytics", status: "completed", publicationId: "pub-0001", metrics: { impressions: 5000, views: 1200, likes: 800, completionRate: 0.6, clickThroughRate: 0.02, revenue: 1200, conversions: 40 }, retrievedAt: "2026-08-14T03:00:00.000Z" }),
  };
    ...
    prepareInput: (step, context) => {
      switch (step.agent) {
        ...
        case "reviewer": {
          ...
          return { requestId: "review-8-24", task: reviewTask, context: { artifact: { kind: prev.kind, artifactId: prev.artifactId, payload: prev.payload } } };
        }
        case "qa": {
          ...
          return { requestId: "qa-8-24", task: qaTask, objective: "Validate the content chain", request: { scope: "content chain", requirements: ["approved"], expectedTests: ["structure"] }, validatedArtifacts: lineage };
        }
        case "thumbnail": {
          const lineage = (context.data.validatedArtifacts ?? []).filter((a) => a.kind !== "qa_report");
          return { requestId: "thumbnail-8-24", objective: "Generate a thumbnail for the approved article", taskDescription: thumbnailTask.description, validatedArtifacts: lineage };
        }
        case "video": {
          const lineage = (context.data.validatedArtifacts ?? [])
            .filter((a) => a.kind !== "qa_report")
            .filter((a) => !(a.kind === "review_report" && a.parentArtifact?.kind === "thumbnail_report"));
          return { requestId: "video-8-24", objective: "Generate a video for the approved article", taskDescription: videoTask.description, validatedArtifacts: lineage };
        }
        case "publisher": {
          const lineage = (context.data.validatedArtifacts ?? [])
            .filter((a) => !(a.kind === "review_report" && (a.parentArtifact?.kind === "thumbnail_report" || a.parentArtifact?.kind === "video_report")));
          return { requestId: "publish-8-24", objective: "Publish the approved content", taskDescription: publishTask.description, validatedArtifacts: lineage };
        }
        case "analytics": {
          const lineage = (context.data.validatedArtifacts ?? [])
            .filter((a) => !(a.kind === "review_report" && (a.parentArtifact?.kind === "thumbnail_report" || a.parentArtifact?.kind === "video_report")));
          return { requestId: "analytics-8-24", objective: "Fetch performance analytics for the published content", taskDescription: analyticsTask.description, validatedArtifacts: lineage };
        }
case "growth": {
          const lineage = (context.data.validatedArtifacts ?? [])
            .filter((a) => !(a.kind === "review_report" && (a.parentArtifact?.kind === "thumbnail_report" || a.parentArtifact?.kind === "video_report")));
          return { requestId: "growth-8-24", objective: "Recommend growth actions from the analytics report", taskDescription: growthTask.description, validatedArtifacts: lineage };
        }
        case "finance": {
          const lineage = (context.data.validatedArtifacts ?? [])
            .filter((a) => !(a.kind === "review_report" && (a.parentArtifact?.kind === "thumbnail_report" || a.parentArtifact?.kind === "video_report")));
          return { requestId: "finance-8-24", objective: "Analyze financial performance", taskDescription: financeTask.description, validatedArtifacts: lineage, financialData: { cost: 600, currency: "USD" } };
        }
        default: return {};
      }
    },
...
describe("Sprint 8.24 — FinanceAgent integration", () => {
  it("— Registry resolves the finance agent", async () => {
    const { registry } = buildHarness();
    strictEqual(registry.has("finance"), true);
    const resolved = await registry.resolve("finance");
    strictEqual(resolved.id, "finance");
  });

  it("— full chain produces a completed finance_report after growth", async () => {
    const { run } = buildHarness();
    const result = await run(FULL);
    strictEqual(result.status, "completed");
    const finance = result.lineage[15];
    strictEqual(finance.kind, "finance_report");
    strictEqual(finance.payload.status, "completed");
    strictEqual(finance.payload.reportId, "finance-8-24");
    strictEqual(finance.payload.contentId, "00000000-0000-4000-8000-0000000000W1");
    strictEqual(typeof finance.payload.confidence, "number");
  });

  it("— finance derives money values only from validated data", async () => {
    const { run } = buildHarness();
    const result = await run(FULL);
    const finance = result.lineage[15].payload;
    strictEqual(finance.revenue, 1200);
    strictEqual(finance.cost, 600);
    strictEqual(finance.profit, 600);
    strictEqual(finance.roi, 1);
    strictEqual(finance.margin, 0.5);
    strictEqual(finance.cpa, 15);
    strictEqual(finance.cpaType, "CPA");
    strictEqual(finance.currency, "USD");
  });

  it("— finance follows the completed analytics and growth artifacts", async () => {
    const { run } = buildHarness();
    const result = await run(FULL);
  ...
  it("— no fabricated numbers: finance revenue matches the analytics revenue", async () => {
    const { run } = buildHarness();
    const result = await run(FULL);
    const analyticsRevenue = result.lineage[13].payload.metrics.revenue;
    const finance = result.lineage[15].payload;
    strictEqual(finance.revenue, analyticsRevenue);
    ok(!Object.prototype.hasOwnProperty.call(finance, "profit") || typeof finance.profit === "number");
  });

  it("— the finance_report chained lineage preserves workflow/correlation lineage", async () => {
    const { run } = buildHarness();
    const result = await run(FULL);
    strictEqual(result.lineage[15].parentArtifact.kind, "growth_report");
  ...
  it("— finance references analytics/growth sources for provenance", async () => {
    const { run } = buildHarness();
    const result = await run(FULL);
    const kinds = result.lineage[15].payload.sourceArtifactReferences.map((r) => r.kind);
    ok(kinds.includes("analytics_report"));
    ok(kinds.includes("growth_report"));
  });

  it("— an analytics failure propagates and blocks finance from claiming numbers", async () => {
    const { run } = buildHarness({
      analyticsProvider: { fetch: async () => ({ providerId: "fake-analytics", status: "failed", error: { code: "RATE_LIMIT", message: "Quota exceeded" } }) },
    });
    const result = await run(FULL);
    strictEqual(result.status, "failed");
    const completed = result.lineage.filter((a) => a.kind === "finance_report" && a.payload.status === "completed");
