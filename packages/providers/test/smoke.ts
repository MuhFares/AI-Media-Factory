/**
 * OpenRouter Provider � Smoke Test
 *
 * Production verification using a real OpenRouter API key.
...
async function main() {
  console.log('=== OpenRouter Provider Smoke Test ===\n');
  ...
  const capabilities = provider.describe(testModel);
  if (capabilities) {
...
if (response && (response.text !== undefined || response.usage)) {
